...
<form
	action="<?php echo JRoute::_('index.php?option=com_easyreservation&view=newreservation'); ?>"
	method="post" name="createReservationForm" id="createReservationForm">

	<label>
        <?php echo JText::_(COM_EASYRESERVATION_NEW_RESERVATION_LABEL_RESERVABLE);?>
	...
	<label>
	<?php echo JText::_(COM_EASYRESERVATION_NEW_RESERVATION_LABEL_DATE);
	      echo JHtml::calendar ( date('Y-m-d',strtotime($this->input_data['start_date'])), 'start_date', 'start_date', '%d.%m.%Y' );
	?>
	</label>

	<label>
	<?php echo JText::_(COM_EASYRESERVATION_NEW_RESERVATION_LABEL_TIME);?>
	...
	<label>
	<?php echo JText::_(COM_EASYRESERVATION_NEW_RESERVATION_LABEL_DURATION);?>
	...
	<label>
	<?php echo JText::_(COM_EASYRESERVATION_NEW_RESERVATION_LABEL_TEXT);?>
	<input type="text" id="reservation_name" name="reservation_name"
	       value="<?php echo $this->input_data['reservation_name']; ?>"
	       maxlength="50"> 
	</label>
	<?php } ?>
	...
	<label>
	<?php echo JText::_(COM_EASYRESERVATION_NEW_RESERVATION_LABEL_ENDDAY);
        if (isset($this->input_data ['end_day'])) {
		echo JHtml::calendar ( date ( 'Y-m-d', strtotime ( $this->input_data ['end_day'] ) ), 'end_day', 'end_day', '%d.%m.%Y' );
	} else {
		echo JHtml::calendar ( "", 'end_day', 'end_day', '%d.%m.%Y' );
	}
	?>
	</label>
