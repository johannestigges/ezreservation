 ...
 */
class EasyReservationModelReservation extends JModelForm {
	/**
	 ...
	 * @var string
	 */
	protected $_context = 'com_easyreservation.reservation';
	...
	protected function populateState() {
		...
		if ((! $user->authorise ( 'core.edit.state', 'com_easyreservation' )) && (! $user->authorise ( 'core.edit', 'com_easyreservation' ))) {
	...
	public function getForm($data = array(), $loadData = true) {
		// Get the form.
		$form = $this->loadForm ( 'com_easyreservation.reservation', 'reservation', array (
				'control' => 'jform',
				'load_data' => true 
		) );
		...
		$id = $this->getState ( 'reservation.id' );
	...
	protected function loadFormData() {
		$data = ( array ) JFactory::getApplication ()->getUserState ( 'com_easyreservation.reservation.data', array () );
		
		$this->preprocessData ( 'com_easyreservation.reservation', $data );
	...
	public function &getItem($pk = null) {
		...
		if (! isset ( $this->_item [$pk] )) {
			try {
				...
				$query->select ( $this->getState ( 'item.select', 'a.*' ) . ',' . $case_when . ',' . $case_when1 )->from ( '#__contact_details AS a' )->
				
				// Join on category table.
				select ( 'c.title AS category_title, c.alias AS category_alias, c.access AS category_access' )->join ( 'LEFT', '#__categories AS c on c.id = a.catid' )->
				
				// Join over the categories to get parent category titles
				select ( 'parent.title as parent_title, parent.id as parent_id, parent.path as parent_route, parent.alias as parent_alias' )->join ( 'LEFT', '#__categories as parent ON parent.id = c.parent_id' )->
				...
				if (is_numeric ( $published )) {
					$query->where ( '(a.published = ' . ( int ) $published . ' OR a.published =' . ( int ) $archived . ')' )->where ( '(a.publish_up = ' . $nullDate . ' OR a.publish_up <= ' . $nowDate . ')' )->where ( '(a.publish_down = ' . $nullDate . ' OR a.publish_down >= ' . $nowDate . ')' );
				}
	 ...
	 */
	protected function getReservationQuery($pk = null) {
		...
		if ($pk) {
			...
			$query->select ( 'id,name,description,reservation_type,user_id,id_reservable,start_time,end_time,start_day,end_day,created' )-> //
from ( '#__ezr_reservation' )-> //
where ( 'id = ' . ( int ) $pk );
			...
			try {
				...
				if (empty ( $result )) {
					throw new Exception ( JText::_ ( 'COM_EASYRESERVATION_ERROR_RESERVATION_NOT_FOUND' ), 404 );
				}
			} catch ( Exception $e ) {
				$this->setError ( $e );
			...
			if ($result) {
				...
				if ($this->getState ( 'params' )->get ( 'show_reservation_list' )) {
				...
				$dispatcher->trigger ( 'onContentPrepareForm', array (
						$form,
						$data 
				) );
				
				// Trigger the data preparation event.
				$dispatcher->trigger ( 'onContentPrepareData', array (
						'com_users.profile',
						$data 
				) );
	 ...
	 *
	 * @param integer $pk
	 *        	Optional primary key of the contact to increment.
	 *        	
